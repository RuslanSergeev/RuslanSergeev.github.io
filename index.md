# Рабочие материалы
[Установка speechbox, решение проблем](work_progress.md)  
[Android studio и проект под Android](AndroidStudio.md)  
[организация удалённой работы](remote_work.md)  
[добавление телеграм-бота в скрипты](telegram_bots.md)  
# Решение задач  
[Решение задачи разбиения датасета](tasks/datasets_splitting.md)  
[Решение задачи дообогащения словаря](tasks/enhancement.md)  
[Задача параллельного обучения](tasks/train_parallell.md)  
[Задача с сабвордами](tasks/subwords.md)  
[Ближайшие планы](tasks/plans.md)  
# Результаты тестов
[Ускорение инференс-прохода под андроид](android_neural_networks.md)  
[устойчивость при аугментации датасета](augmentation_tests.md)  
# Мои CV
[CV на роль ADCS Software Engineer](CV/Ruslan_Sergeev_CV.pdf)  
[Cover letter для компании JPL](CV/Ruslan_Sergeev_Letter_JPL.PDF)  
# Мои патенты
[Патент на стенд для наземной инфраструктуры отработки](Patents/Ruslan_Sergeev_ground_control_center_patent.pdf)  
[Патент на систему управления двигателями](Patents/Ruslan_Sergeev_PMSM_patent.pdf)  
